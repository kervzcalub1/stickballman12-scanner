...
## Stack (one line)
React 18 + Vite (all UI in `src/App.jsx`); Node handlers in `api/**/*.js` served
by `server.mjs` (Express) + Vite dev middleware; Postgres via the `sql` shim in
...
## Working agreements
- Match the surrounding code's style; keep `src/App.jsx` components consistent
  with existing ones. Run `npm run build` to verify before declaring done.

...
// different size, which may genuinely still be -100. GET reports how many that would be
...
import { getItemByVin, findSkuSiblings, setItemsSku, dbConfigured } from '../_lib/db.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const scope = body.scope === 'same_upc' ? 'same_upc' : 'one';
    const targets = scope === 'same_upc' ? siblings : siblings.filter((s) => s.vin === item.vin);

...
const VIDEOS = [
  ...
  // resetting the data. Every screen is still shown; only the final write is withheld.
  {
    ...
    beats: [
      ...
      { say: 'If the shipment truly arrived without one, tick No tracking number instead.',
        at: '.no-track-check' },
      { say: 'Hand-delivered, local pickup, a supplier who never sent one. It is recorded on the batch.' },
      { say: 'Only for a shipment that really has none — not for a label you have not read yet.' },
      { say: 'For a single box, leave boxes expected at one.' },
      { say: 'Otherwise neither field is optional — the server rejects a batch without them.' },
  ...
  },
  {
    ...
    beats: [
      ...
      { say: 'This page prints them for the queue in bulk.' },
      { say: 'For a single pair from a VIN, a UPC or a SKU, use the Box Labels page instead.' },
      { say: 'Same label, two ways in — whichever suits the job in front of you.' },
    ],
  },
  {
    // The whole point of this one is the FAILURE mode people hit: they scan the UPC,
    // the outside catalogue has never heard of the shoe, and they conclude the tool is
    // broken. So it demonstrates the lookup finding the pair in OUR stock, and ends on
    // the real printed label rather than on the button that makes it.
    // Non-destructive: opening the label sheet only builds a PDF preview.
    id: 'box-labels', role: 'warehouse', title: 'Print a box label for a pair with no box',
    path: '/box-labels',
    beats: [
      { say: 'A shoe with no box cannot be sold, and a shoe with an unreadable box cannot be scanned.' },
      { say: 'This page rebuilds the box label, so the pair behaves like any other again.' },
      { say: 'One field takes all three things you might be holding.', at: '.searchrow input' },
      { say: 'The VIN sticker on the shoe. The old box UPC. Or a SKU you type by hand.' },
      { say: 'Here is a pair from the No Box queue. We will use its box UPC.',
        type: ['.searchrow input', process.env.SOP_SPEC_NOBOX_UPC || '198481311903'], zoom: false },
      { say: 'Find.', click: 'button:has-text("Find")', hold: 2600 },
      { say: 'From your inventory. That line matters.', waitFor: '.dcard-name', at: '.dcard-top .muted.sm' },
      { say: 'We look in our own stock BEFORE any outside catalogue.' },
      { say: 'Old stock, in-store buys, anything typed in by hand — the catalogue has never heard of them.' },
      { say: 'Looking outside first is why this used to say no product found for a shoe we own.' },
      { say: 'And because it found us, it lists the actual pairs we hold.', at: '.boxlbl-unit', zoom: false },
      { say: 'Pick the one in your hand. Do not create a second VIN for a pair we already have.',
        at: '.boxlbl-unit:has-text("Bought Without Box") .btn' },
      { say: 'Use this VIN.', click: '.boxlbl-unit:has-text("Bought Without Box") .btn', hold: 2600 },
      { say: 'Now it is that exact pair — its VIN, its size, its status.', waitFor: '.vin', at: '.dcard-top .vin' },
      { say: 'Print box label.', at: 'button:has-text("Print box label")' },
      { say: 'Let us look at what comes out.', click: 'button:has-text("Print box label")', hold: 3000 },
      { say: 'One dialog. Choose your label stock, and that is the only decision.',
        waitFor: '.print-dialog', at: '.print-stock select' },
      { say: 'Print. On a phone it goes straight to the share sheet — tap Print there.',
        at: '.print-dialog .btn.primary' },
      { say: 'What comes out is a real shoe-box label: vertical UPC barcode down the left, name, size and SKU beside it.' },
      { say: 'Stick it on the replacement box, and the pair scans normally from then on.',
        click: '.print-dialog button:has-text("Cancel")', hold: 2200 },
      { say: 'Two things that catch people out.' },
      { say: 'If the pair has no UPC on file, you will be asked for one before it prints.' },
      { say: 'Read it off the tongue label inside the shoe. It saves to the pair, so you are asked once.' },
      { say: 'And give it a VIN is only for old stock that is genuinely not in the system yet.' },
      { say: 'If the page already listed matching pairs, one of them IS your shoe. Use it.' },
      { say: 'Creating a second VIN counts the same shoe twice, and the stock figure starts lying.' },
    ],
  },
  {
  ...
  },
  {
    ...
    beats: [
      ...
      { say: 'One dialog, one decision: your label stock. C R 80 card is the default.',
        waitFor: '.print-dialog', at: '.print-stock select' },
      ...
      { say: 'Then Print. Labels come out as an exact-size PDF, one label per page.',
        at: '.print-dialog .btn.primary' },
      { say: 'Not a browser print. That is what makes them come out the right size on a label printer.' },
      { say: 'On a phone, Print hands it to the share sheet — tap Print there. On a computer the print dialog opens itself.', key: 'Escape' },
  ...
  },
  {
    ...
    beats: [
      ...
      { say: 'Scan each VIN — with a gun, or the camera. Typing works too.',
        type: ['.searchrow input', process.env.SOP_SPEC_VIN || 'SBM-260622-000001'] },
  ...
  {
    id: 'mark-shipped', role: 'warehouse', title: 'Mark pairs shipped (bulk scan-out)',
    path: '/shipped',
    beats: [
      { say: 'Scan-out is built for a three hundred pair day. Scan, scan, scan — then submit once.' },
      { say: 'These four numbers are the whole dashboard.', at: '.scanout-stats', zoom: false },
      { say: 'Scanned, remaining, errors, and the last pair that went on.' },
      { say: 'Remaining is pairs already marked sold that have not gone out yet.',
        at: '.scanout-stat:nth-child(2)' },
      { say: 'Scan each VIN. The scanner never closes and nothing pops up in between.',
        type: ['.searchrow input', process.env.SOP_SPEC_VIN || 'SBM-260622-000001'] },
      { say: 'Add.', click: 'button:has-text("Add")', hold: 2600 },
      { say: 'Listen rather than watch. One short high blip means it went on the list.',
        waitFor: '.inv-table', at: '.scanout-last .vin' },
      { say: 'Two low buzzes mean it did not, and the banner tells you why.' },
      { say: 'You are holding a box and a scanner gun. The sound is the feedback that reaches you.' },
      { say: 'Mute it with the speaker button if the room needs quiet.', at: '.topbar button[aria-pressed]' },
      { say: 'Anything refused goes in the failed scans list, with its reason and the time.' },
      { say: 'It stays there for the whole session — the next scan does not wipe it.' },
      { say: 'Usually you scanned the product UPC instead of the VIN, or the pair already shipped.' },
      { say: 'Scanned the wrong box? Undo last takes it straight back off.',
        at: 'button:has-text("Undo last")' },
      { say: 'And it can be scanned again immediately.' },
      { say: 'Nothing is written until you submit. That is what makes a mis-scan harmless.' },
      { say: 'When the trolley is done, Save.', click: 'button:has-text("Save")', hold: 2600 },
      { say: 'The VINs are read back to you one final time.',
        waitFor: '.confirm-list', at: '.confirm-list' },
      { say: 'Confirm, and you get the session summary — how many went out, how many errored.',
        key: 'Escape' },

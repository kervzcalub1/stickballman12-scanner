...
const VIDEOS = [
  ...
  },
  {
    ...
    beats: [
      ...
      { say: 'A real shoe-box label. Vertical UPC barcode down the left, exactly like the original.',
        waitFor: '.rlabel.boxlabel', at: '.blabel-bc' },
      { say: 'Name, size and SKU on the right, so a human can read it without scanning.',
        at: '.blabel-text' },
      { say: 'Choose your label stock, then print. It comes out as an exact-size PDF.',
        at: '.label-toolbar select' },
      { say: 'Stick it on the replacement box, and the pair scans normally from then on.',
        click: 'button:has-text("Close")', hold: 2200 },

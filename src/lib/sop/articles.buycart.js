...
export const BUYCART_ARTICLES = [
  {
    ...
    steps: [
      { do: 'A BUYER opens a request and asks about pairs as they find them — SKU, size and the shelf price. Each pair goes to the approvers (and the Telegram group) the moment it is added; the desk prices it and says Buy, Watch or Pass.' },
      { do: 'When they have nothing more to add they CLOSE the request. It will not close without saying what is being bought and which store, and the gift cards wait for it.' },
...

  {
    ...
    area: 'buying',
    roles: ['supplier'],
    summary: 'Open a request, ask about each pair as you find it, close the request when you are done — then spend the cards and send the receipt.',
    ...
    steps: [
      ...
      { do: 'Ask the desk about this. Repeat for every pair — each one goes straight to the approvers, and their answers land on your screen while you are still in the shop.', note: 'The desk prices it against Alias and StockX and makes the call — you will not see a Buy or Pass on your screen, so ask about anything you think is worth it. A pair nobody has answered on yet can be removed with the ×.' },
      { do: 'Close the request when you have nothing more to add. That is what sends it for the gift cards — the desk cannot fund a list that is still growing.', note: 'Found one more on the way out? Re-open to add more. The approvers are told, and any cards already issued stay yours to spend; a top-up follows once the new pairs are approved.' },
    ...
    rules: [
      ...
      'Every pair you add is asked about immediately — there is no send button. Close the request when you are done; re-open it if you find more, until the receipt is in.',
      ...
      'The cards are sized on the sticker price PLUS sales tax. If a card still comes up short, re-open the request and ask about the extra — never pay the difference yourself.',
    ],
...

  {
    ...
    rules: [
      'The approved total is the SHELF price of every approved pair — no discounts assumed. The cards get sized against that PLUS the sales tax on the cost stack ("to fund").',
...

  {
    ...
    steps: [
      ...
      { do: 'Watch the funding line. It says what the cards total against what has to be funded — the approved total plus sales tax — and how much is still short.', note: 'No card can be recorded while the buyer is still adding. The request says "Buyer still adding" until they close it; once closed and every line decided, the form appears.' },
    ...
    rules: [
      ...
      'The cards must cover the approved total plus tax before anything is released. The button stays disabled and names the shortfall.',
      'A buyer can re-open a funded request to ask about more. The cards already issued stay out; once the new pairs are approved and the list is closed again, record a top-up card for the difference.',

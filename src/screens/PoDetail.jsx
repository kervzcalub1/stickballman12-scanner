            ...
            {boxes.map((box) => {
              ...
              return (
                <div key={box.id} className={`card po-lbl${isReplacement ? ' replacement' : ''}`}>
                  ...
                  {/* The carrier is already on the line above, so it is repeated only
                      when tracking came back with a different one than the label claims. */}
                  <DeliveryStatus box={box} carrierOf={(b) => carrierName(b.carrier_key)} />
